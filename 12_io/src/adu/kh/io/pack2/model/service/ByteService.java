package adu.kh.io.pack2.model.service;
...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		try {
			...
			sb.append("~!@#$%^&*()_+?\n");
			sb.append("가나다라마바사아\n");
			sb.append("와 자바가 너무 재밌다~!\n");
			sb.append("선생님 저 공부 열심히 할게요!!");
			sb.append("진짜로!!!");
			sb.append("정말로~!!");
			sb.append("화이팅!!!");
			
			// StringBulder -> String 변환
			String content = sb.toString();
			
			/* 출력방법 1 : 한 글자(2byte)씩 출력
	...
	public void bufferedFileByteOutput() {
		...
		try {
			...
			bos.flush(); // 스트림 안에 남아있는 데이터 밀어내기(쏟아내기)
	...
	public void fileByteInput2() {
		...
		try {
			...
			/* 방법 2. 파일에서 저장된 모든 byte 값을 다 읽어와
			 *         byte[] 형태로 반환 받기
	...
	public void bufferedFileByteInput() {
		...
		try {
			...
			// 보조스트림만 close()해도 기반 스트림도 같이 close
			try {
	 ...
	 *           : fileOuputStream + BuffereadOuputStream
	 * + Scammer 대신 BufferedReader 이용
	 **/
