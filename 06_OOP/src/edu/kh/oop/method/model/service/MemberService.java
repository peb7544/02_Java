...
public class MemberService {
	...
	public void displayMenu() {
		...
		do {
			...
			case 3 : System.out.println( /* 회원 정보 조회용 기능 호출 */ selectMember() ); break;
			case 4 : 
	...
	public String selectMember() {
		...
		/* 1) 로그인 여부 확인
		 * 로그인 안했을 대 "로그인 후 이용해주세요" 리턴
		 * 
		 * 2) 로그인이 되어있는 경우
		 * 회원 정보를 출력할 문자열을 만들어서 반환(retrun)
		 * 단, 비밀번호 제외
		 * 
		 * 이름 : 홍길동
		 * 아이디 : user01
		...
		if(loginMember == null) { // 1) 로그인 여부 확인
	...
	public int updateMember() {
		...
		if(inputPw.equals(loginMember.getMemberPw())) {
			...
			// -> 비밀번호가 같은 경우, 로그인한 회원의 이름, 나이 정보를 입력받은 값으로 변경 후 1 반환
